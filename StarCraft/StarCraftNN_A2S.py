...
def GetData(dir_name):
    try:
        ...
        print("-I- channeled state shape: "+str(channeled_state.shape))
        """
        actios_txt = open("/home/data/starcraft/replay_data/"+dir_name+"/actions.txt")
        actions = list()
        for idx, line in enumerate(actios_txt):
            if idx%3 == 1:
                actions.append(int(line))
    
        
        return record_length, channeled_state, actions
    except:
        return 0,[0]
        ...
        for embed in EMBEDDINGS:
            ...
            for epoch in Epochs:
                ...
                for batch in BATCHS:         
                    ...
                    name="StarCraftNN-A2S_CONTEXT="+str(CONTEXT_SIZE)+"_LENGTH="+str(length)+"_EMBEDINNGS="+str(embed)+"_EPOCHS="+str(epoch)+"_BATCH="+str(batch) 
                    print("\n-I- Running name:  \n\t" + name)
                    ...
                    for i in range(data_len):
                        dir_list.append( directories[int(np.random.randint(26592))] )
                    ...
                    try:
                        for ep in range(EPOCHS):
                            ...
                            for dir_idx in range(data_len):                                             
                                ...
                                state_length, data_states, actions = GetData(dir_list[dir_idx])

                                print(actions)

                                print("-I- Dir name: "+str(dir_list[dir_idx]))
                                if state_length == 0:
                                    continue
                                model.zero_grad()
                            ...
                            print(total_loss)
                            print("****************************************")
                            losses.append(np.log(total_loss))
