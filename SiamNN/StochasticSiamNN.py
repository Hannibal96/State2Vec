                    ...
                    for nn_type in NN_TYPE:
                        for epoch in Epochs:
                            for batch in BATCHS:                            
                                ...
                                name="StochasticNN_DATA="+str(data)+"_SIZE="+str(size)+"_CONTEXT="+str(CONTEXT_SIZE)+"_LENGTH="+str(length)+"_EMBEDINNGS="+str(embed)+"_NN="+str(nn_type)+"_EPOCHS="+str(epoch)+"_BATCH="+str(batch) 
                                print("\n====Running name:  \n\t" + name)
                                ...
                                try:
                                    for ep in range(EPOCHS):
                                        ...
                                        data_img = game_of_life(SIZE, 20)
                                        
                                        for sub_ep in range(1):
                                          for idx, state in enumerate(data_img[:-1]):
                                              ...
                                              false_img = game_of_life(SIZE, 1)
                                              mask_param = 2*np.sum(data_img[true_idx])/(SIZE*SIZE)
                                              if mask_param > 1:
                                                  mask_param = 1
                                              false_mask = np.random.binomial(1, mask_param, size=(SIZE,SIZE))

                                              classification_neg = model( torch.tensor([[false_img[0]*false_mask]]).float().cuda(), torch.tensor([[state]]).float().cuda() )
