                    ...
                    for nn_type in NN_TYPE:
                        for epoch in Epochs:
                            for batch in BATCHS:                            
                                ...
                                name="RandNN_DATA="+str(data)+"_SIZE="+str(size)+"_CONTEXT="+str(CONTEXT_SIZE)+"_LENGTH="+str(length)+"_EMBEDINNGS="+str(embed)+"_NN="+str(nn_type)+"_EPOCHS="+str(epoch)+"_BATCH="+str(batch) 
                                print("\n====Running name:  \n\t" + name)
                                    ...
                                    for ep in range(EPOCHS):
                                        ...
                                        for idx, state in enumerate(data_img):
                                            while True:                            # -I- Generate index for the true and false samples
                                                ...
                                                if not (true_idx < 0 or true_idx >= length):
                                                    break
